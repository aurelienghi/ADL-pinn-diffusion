...
class Auxilliary(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear1 = nn.Linear(2, 4) # 4 inputs, one layer of 4 nodes
        self.linear2 = nn.Linear(4, 2) # 2 latent dims, one layer of 4 nodes: mean
...
class Autoencoder(nn.Module):
    ...
    def forward(self, x, t):
        ...
        y = self.aux(z)
        return self.decoder(z), torch.cat([x,t_bar],dim=1), y
...
def train(autoencoder, data, epochs=200):
    ...
    for epoch in range(epochs):
        for x_0, y in data:
            ...
            x_t, eps = fwd(x_0, t)
            eps_hat, x_bar, y_hat = autoencoder(x_t,t) #fetch output
...
def sampling(T,autoencoder): #start with x_t = x_T (normally distributed)
    ...
    physics = autoencoder(x_t,torch.tensor([0]))[2]
    return x_t, physics
    ...
    for kk in range(5):
        ...
        with torch.no_grad():
            for ii in range(len(y_test)):
                test_pred[ii,:] = diff(torch.tensor(np.array([x_test[ii,:]]),dtype=torch.float32),torch.tensor([0]))[2]
