...
def physicsLoss(in_val,out_val):
    gradient = grad(out_val, in_val)
    loss = sum(gradient[0][0])
    return loss
...
def train(autoencoder, conditional, data, epochs=40):
    ...
    for epoch in range(epochs):
        for x_0, y, c in data:
            ...
            loss2 = 0.5*(w*func.l1_loss(eps_c[0], eps_hat_c[0,:9]) + (1-w)*func.l1_loss(y, y_hat_c)) + 0.5*physics_loss
...
def sample_step(x_t,c,t,autoencoder,conditional):
    ...
    eps_cond, x_cond = conditional(torch.cat([x_t,c],dim=1),torch.tensor([t]))
    eps_pred = (1+cW)*eps_cond - cW*eps_pred
...
def optfn(x_norm,y_norm,c_norm):
    ...
    for kk in range(5):
        ...
        diff, conditonal = train(diff, conditional, traindata)
...
# Normalise
x_norm = load_data('aero_vals.csv')
y_norm = load_data('aero_train.csv')
c_norm = load_data('y_vals.csv')

# HOW TO KNOW WHEN TO STOP TRAINING

mse = optfn(x_norm,y_norm,c_norm[1:,:]).mean()
print(mse)