...
import env_ATR_2 as env
import cost_ATA_67 as cost
...
def despt(xvals):
    ...
    configs, analyses = full_setup(xvals)
    ...
    # Post-process
    # Fuel burn
    # Flight time as fraction of total time
    fuel_1 = results.segments['climb_1'].conditions.weights.total_mass[0]-results.segments['climb_1'].conditions.weights.total_mass[-1]
    fuel_2 = results.segments['climb_2'].conditions.weights.total_mass[0]-results.segments['climb_2'].conditions.weights.total_mass[-1]
    fuel_3 = results.segments['climb_3'].conditions.weights.total_mass[0]-results.segments['climb_3'].conditions.weights.total_mass[-1]
    fuel_4 = results.segments['cruise'].conditions.weights.total_mass[0]-results.segments['cruise'].conditions.weights.total_mass[-1]
    fuel_5 = results.segments['descent_1'].conditions.weights.total_mass[0]-results.segments['descent_1'].conditions.weights.total_mass[-1]
    fuel_6 = results.segments['descent_2'].conditions.weights.total_mass[0]-results.segments['descent_2'].conditions.weights.total_mass[-1]
    fuel_7 = results.segments['descent_3'].conditions.weights.total_mass[0]-results.segments['descent_3'].conditions.weights.total_mass[-1]
    fuel_8 = results.segments['descent_4'].conditions.weights.total_mass[0]-results.segments['descent_4'].conditions.weights.total_mass[-1]
    fuel_9 = results.segments['descent_5'].conditions.weights.total_mass[0]-results.segments['descent_5'].conditions.weights.total_mass[-1]

    fuel = np.array([fuel_1,fuel_2,fuel_3,fuel_4,fuel_5,fuel_6,fuel_7,fuel_8,fuel_9])

    alt_1 = (results.segments['climb_1'].altitude_end)/2
    alt_2 = (results.segments['climb_1'].altitude_end-results.segments['climb_2'].altitude_end)/2
    alt_3 = (results.segments['climb_2'].altitude_end-results.segments['climb_3'].altitude_end)/2
    alt_4 = results.segments['climb_3'].altitude_end
    alt_5 = (results.segments['climb_3'].altitude_end-results.segments['descent_1'].altitude_end)/2
    alt_6 = (results.segments['descent_1'].altitude_end-results.segments['descent_2'].altitude_end)/2
    alt_7 = (results.segments['descent_2'].altitude_end-results.segments['descent_3'].altitude_end)/2
    alt_8 = (results.segments['descent_4'].altitude_end-results.segments['descent_5'].altitude_end)/2
    alt_9 = (results.segments['descent_5'].altitude_end)/2

    altitude = np.array([alt_1,alt_2,alt_3,alt_4,alt_5,alt_6,alt_7,alt_8,alt_9])

    aspeed_1 = results.segments['climb_1'].air_speed
    aspeed_2 = results.segments['climb_2'].air_speed
    aspeed_3 = results.segments['climb_3'].air_speed
    aspeed_4 = results.segments['cruise'].air_speed
    aspeed_5 = results.segments['descent_1'].air_speed
    aspeed_6 = results.segments['descent_2'].air_speed
    aspeed_7 = results.segments['descent_3'].air_speed
    aspeed_8 = results.segments['descent_4'].air_speed
    aspeed_9 = results.segments['descent_5'].air_speed

    speed = np.array([aspeed_1,aspeed_2,aspeed_3,aspeed_4,aspeed_5,aspeed_6,aspeed_7,aspeed_8,aspeed_9])
    fuel_burn = configs.base.mass_properties.takeoff - results.segments['descent_5'].conditions.weights.total_mass[-1]
    seats = 170

    d1   = results.segments['climb_1'].altitude_end/np.tan(np.arcsin(results.segments['climb_1'].climb_rate/results.segments['climb_1'].air_speed))
    d2   = (results.segments['climb_2'].altitude_end-results.segments['climb_1'].altitude_end)/np.tan(np.arcsin(results.segments['climb_2'].climb_rate/results.segments['climb_2'].air_speed))
    d3   = (results.segments['climb_3'].altitude_end-results.segments['climb_2'].altitude_end)/np.tan(np.arcsin(results.segments['climb_3'].climb_rate/results.segments['climb_3'].air_speed))
    d4   = results.segments['cruise'].distance
    d5   = (results.segments['climb_3'].altitude_end-results.segments['descent_1'].altitude_end)/np.tan(np.arcsin(results.segments['descent_1'].descent_rate/results.segments['descent_1'].air_speed))
    d6 = (results.segments['descent_1'].altitude_end-results.segments['descent_2'].altitude_end)/np.tan(np.arcsin(results.segments['descent_2'].descent_rate/results.segments['descent_2'].air_speed))
    d7 = (results.segments['descent_2'].altitude_end-results.segments['descent_3'].altitude_end)/np.tan(np.arcsin(results.segments['descent_3'].descent_rate/results.segments['descent_3'].air_speed))
    d8 = (results.segments['descent_3'].altitude_end-results.segments['descent_4'].altitude_end)/np.tan(np.arcsin(results.segments['descent_4'].descent_rate/results.segments['descent_4'].air_speed))
    d9 = (results.segments['descent_4'].altitude_end-results.segments['descent_5'].altitude_end)/np.tan(np.arcsin(results.segments['descent_5'].descent_rate/results.segments['descent_5'].air_speed))

    mission_range = (d1+d2+d3+d4+d5+d6+d7+d8+d9)/1609.344 #in miles
    
    block_time = (d1/aspeed_1+d2/aspeed_2+d3/aspeed_3+d4/aspeed_4+d5/aspeed_5+d6/aspeed_6+ \
         d7/aspeed_7+d8/aspeed_8+d9/aspeed_9)/3600 #in hours
    block_speed = mission_range/block_time
    #tow = configs.base.mass_properties.takeoff
    owe = configs.base.mass_properties.operating_empty
    thrust = 27000/2
    life = 30
    cycles_per_day = 2
    tf = d4/aspeed_4/3600
    U = 8760-cycles_per_day*block_time*365

    desflight = env.flight(cycles_per_day*365,fuel,abs(altitude)*3.34,speed)

    RFimpact = env.ATRcalc(30,100,desflight,r=0.03)/(seats*life*365*cycles_per_day*mission_range)
    casm = cost.quick_cost(fuel_burn[0]*2.2046,seats,mission_range,block_speed,owe*2.2046,thrust,tf,U)

    cl   = np.mean(results.segments["cruise"].conditions.aerodynamics.lift_coefficient[:,0,None])
    cd   = np.mean(results.segments["cruise"].conditions.aerodynamics.drag_coefficient[:,0,None])

    result_array = [RFimpact,casm,cl,cd]
...
def full_setup(xvals):
    ...
    mission  = mission_setup(configs_analyses,xvals)
...
def vehicle_setup(xvals):
    ...
    wing.sweeps.quarter_chord    = xvals[4] * Units.deg
    wing.thickness_to_chord      = 0.1
    wing.taper                   = 0.1
    wing.spans.projected         = xvals[1] * Units.meter
    rootchord = 2*124.862/(xvals[1]*(1+xvals[2]))
    wing.chords.root             = rootchord* Units.meter
    wing.chords.tip              = rootchord*xvals[2] * Units.meter
    wing.chords.mean_aerodynamic = rootchord*2/3*((1+xvals[2]+xvals[2]**2)/(1+xvals[2])) * Units.meter
    ...
    wing.areas.reference         = 124.862*xvals[3] * Units['meters**2']  
...
def mission_setup(analyses,xvals):
    ...
    segment.altitude_end = xvals[6] * Units.km
    ...
    segment.air_speed  = xvals[5] * Units['m/s'] #230.412
...
if __name__ == '__main__':
    matrix = np.array([0,0])
    for iaspect in np.linspace(7,17,5):
        for ispan in np.linspace(15,35,5):
            for itaper in np.linspace(0.1,0.9,5):
                for iareaH in np.linspace(0.2,0.5,5):
                    for isweep in np.linspace(0,30,5):
                        r = despt([iaspect,ispan,itaper,iareaH,isweep,230.,10.])
                        print(r)
                        breakpoint()
                        matrix = np.vstack((matrix, r))
                        #print(r)

    np.savetxt("aero_train.txt", matrix, delimiter = ", ")

#WARNING: AR CHANGING NOTHING??