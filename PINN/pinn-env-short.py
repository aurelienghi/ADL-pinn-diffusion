...
phys_pred_t = np2torch(np.multiply(np.divide(phys_pred,np.transpose(np.tile(range_y,(500,7,1)),(0,2,1))),x_std))
print("Phys end")
...
for physweight in [1.0,0.5,0.1,0.05,0.01,0.005,0.001,0.0005,0.0001,0.0]:
    net = MLP()
    aux = MLP2()
    losses = trainfn(net, aux, x_norm[numbers[:bb],:], y_norm[numbers[:bb],:],physin_t, phys_pred_t, lr, physweight)
...
np.savetxt("MSEConditions_co3.txt",hyperparams)
np.savetxt("MSEenv_co3.txt",mse_env)
np.savetxt("MSEcost_co3.txt",mse_cost)